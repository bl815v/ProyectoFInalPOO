...
public class PanelRegisterPareja extends JPanel {
	...
	private JLabel etitulo, esubtitulo, enombre, esubnombre, egenero, esubgenero, ecorreo, esubcorreo, edia, ehora,
			esubdia, esubhora;
	private JLabel eusuario, esubusuario, eclave, esubclave, erepetir, esubrepetir, ecredito, esubcredito, esede,
			esubsede;
	private JComboBox<String> lista_genero, lista_sedes, lista_dias;
	private JTextField tnombre, tcorreo, tusuario, tcredito, thora;
	...
	public void inicializarComponentes() {
		...
		edia = Estandar.labelNegro("Dia");
		Izquierda.add(edia);
		esubdia = Estandar.labelGris("Ingrese el dia de compra");
		Izquierda.add(esubdia);

		JPanel panelAuxdia = new JPanel(new BorderLayout());
		panelAuxdia.setAlignmentX(LEFT_ALIGNMENT);
		panelAuxdia.setMaximumSize(new Dimension(250, 30));
		lista_dias = new JComboBox<String>();
		lista_dias.addItem("");
		lista_dias.addItem("Lunes");
		lista_dias.addItem("Martes");
		lista_dias.addItem("Miercoles");
		lista_dias.addItem("Jueves");
		lista_dias.addItem("Viernes");
		lista_dias.addItem("Sabado");
		lista_dias.setFont(new Font("Arial", Font.PLAIN, 13));
		lista_dias.setForeground(new Color(92, 92, 102));
		lista_dias.setActionCommand("LISTAdias");
		panelAuxdia.add(lista_dias, BorderLayout.CENTER);
		Izquierda.add(panelAuxdia);
		Izquierda.add(Estandar.Espacio());
		...
		ehora = Estandar.labelNegro("Asignar hora de compra");
		Derecha.add(ehora);
		esubhora = Estandar.labelGris("Ingrese la hora (6-18):");
		Derecha.add(esubhora);

		JPanel panelAuxThora = new JPanel(new BorderLayout());
		panelAuxThora.setMaximumSize(new Dimension(600, 30));
		thora = Estandar.campoTexto();
		panelAuxThora.add(thora, BorderLayout.CENTER);
		Derecha.add(panelAuxThora);
		Derecha.add(Estandar.Separador());
		...
		bsiguiente = Estandar.boton("REGISTRAR");
	...
	public JLabel getEdia() {
		return edia;
	}

	public void setEdia(JLabel edia) {
		this.edia = edia;
	}

	public JLabel getEhora() {
		return ehora;
	}

	public void setEhora(JLabel ehora) {
		this.ehora = ehora;
	}

	public JLabel getEsubdia() {
		return esubdia;
	}

	public void setEsubdia(JLabel esubdia) {
		this.esubdia = esubdia;
	}

	public JLabel getEsubhora() {
		return esubhora;
	}

	public void setEsubhora(JLabel esubhora) {
		this.esubhora = esubhora;
	}

	public JComboBox<String> getLista_dias() {
		return lista_dias;
	}

	public void setLista_dias(JComboBox<String> lista_dias) {
