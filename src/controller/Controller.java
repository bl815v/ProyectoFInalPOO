	...
	public void actionPerformed(ActionEvent e) {
		...
		if (comando.equals("BUSUARIOS_ADMIN")) {
			...
			for (Usuario usuario : lista.getListadeUsuarios()) {
				...
				if (!usuariosAgregados.contains(userName)) {
					Object[] fila = { usuario.getNombre(), userName, usuario.getRol(), usuario.getCorreo(),
							usuario.getGenero(), usuario.getCredito(), usuario.getDeuda() };
					modelU.addRow(fila);
					usuariosAgregados.add(userName);
				}
			}
			...
			if (filaSeleccionada != -1) {
				...
				if (usuarioSeleccionado != null) {
					...
					int confirmar = JOptionPane.showOptionDialog(null,
							"Desea elminar a " + usuarioSeleccionado.getNombre(), "Confirmar eliminacion",
		...
		if (comando.equals("BSOLICITUDES_ADMIN")) {
			...
			for (Usuario usuario : usuariosExistentes) {
				if (usuario.getSobrecupo() > 0) {
					String userName = usuario.getUser();
					if (!usuariosAgregados.contains(userName)) {
						Object[] fila = { usuario.getNombre(), userName, usuario.getCorreo(), usuario.getCredito(),
								usuario.getDeuda(), usuario.getSobrecupo() };
		...
		if (comando.equals("BSEDES_ADMIN")) {
			...
			for (Sede sede : listaSedes.getListadeSedes()) {
				...
				if (!sedesAgregadas.contains(sedeNombre)) {
					Object[] fila = { sedeNombre, sede.getDireccion() };
		...
		if (comando.equals("EliminarSedeADMIN")) {
			...
			if (filaSeleccionada != -1) {
				...
				if (sedeSelec != null) {
					Object[] opciones = { "Sí", "No" };
					int confirmar = JOptionPane.showOptionDialog(null, "Desea elminar la " + sedeSelec.getNombre(),
							"Confirmar eliminacion", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE, null,
							opciones, opciones[1]);
					if (confirmar == JOptionPane.YES_OPTION) {
		...
		if (comando.equals("bREGISTRAR")) {
			...
			if (vnombre && vusuario && vgenero && vcorreo && vclave && vcorreoR && vusuarioR) {
				...
				if (!Arrays.equals(clave, repetir)) {
					...
					if (respuesta) {
						volver();
						Estandar.MensajeInformacion(
								"Se ha registrado exitosamente!\n\nSe le ha asignado un cupo de $" + cupo + " pesos",
								"Registro exitoso");
					} else {
						Estandar.MensajeError("Fallo al registrar");
					}
		...
		if (comando.equals("LISTAgenero")) {
			...
			Estandar.revisarJComboBoxString(vInicial.getPr().getLista_genero(), vInicial.getPr().getEsubgenero(),
					"Ingrese su genero:");
		}
		...
		if (comando.equals("bABONAR")) {
			...
			if (vCliente.getPa().getTmonto().getText().equals("") || monto <= 0 || monto > usuario.getDeuda()) {
				...
				Estandar.MensajeInformacion("Ha abonado $" + vCliente.getPa().getTmonto().getText() + " pesos",
						"Abono exitoso");
		...
		if (comando.equals("LISTAsedes")) {
			...
			Estandar.revisarJComboBoxString(vCliente.getPrp().getLista_sedes(), vCliente.getPrp().getEsubsede(),
					"Ingrese la sede:");
		}

		if (comando.equals("LISTAgeneropareja")) {
			vCliente.getPrp().getLista_genero().removeItem(" ");
			Estandar.revisarJComboBoxString(vCliente.getPrp().getLista_genero(), vCliente.getPrp().getEsubgenero(),
					"Ingrese su genero:");
		}
		if (comando.equals("bSiguiente")) {
			boolean vnombre, vusuario, vgenero, vcorreo, vclave, vcorreoR, vusuarioR, vcredito, vsede, vdia, vhora,
					vhorarioR;
			...
			if (vCliente.getPrp().getTcredito().getText().equals("") || credito <= 0
			...
			int hora = 0;
			try {
				hora = Integer.parseInt(vCliente.getPrp().getThora().getText());
			} catch (NumberFormatException n) {
				vCliente.getPrp().getEsubhora().setText("Debe escribir una hora");
				vCliente.getPrp().getEsubhora().setForeground(Color.RED);
				vhora = false;

			}
			if (vCliente.getPrp().getThora().getText().equals("") || hora < 6 || hora > 18) {
				vCliente.getPrp().getEsubhora().setText("Debe escribir una hora valida");
				vCliente.getPrp().getEsubhora().setForeground(Color.RED);
				vhora = false;
			}
			...
			if (vCliente.getPrp().getLista_dias().getSelectedItem().equals("")) {
				vCliente.getPrp().getEsubsede().setText("Seleccione un día");
			...
			if (lista.correoRepetido(vCliente.getPrp().getTcorreo().getText())
					|| listapareja.correoRepetido(vCliente.getPrp().getTcorreo().getText())) {
			...
			if (lista.usuarioRepetido(vCliente.getPrp().getTusuario().getText())
					|| listapareja.usuarioRepetido(vCliente.getPrp().getTusuario().getText())) {
			...
			if (listapareja.tieneOtraParejaConMismoDiaYHora(usuario,
					(String) vCliente.getPrp().getLista_dias().getSelectedItem(),
					Integer.parseInt(vCliente.getPrp().getThora().getText()))) {
				vCliente.getPrp().getEsubhora().setText("Horario repetido");
				vCliente.getPrp().getEsubhora().setForeground(Color.RED);
				vCliente.getPrp().getEsubdia().setText("Horario repetido");
				vCliente.getPrp().getEsubdia().setForeground(Color.RED);
				vhorarioR = false;
			} else {
				vhorarioR = true;
			}
			...
			if (vnombre && vusuario && vgenero && vcorreo && vclave && vcorreoR
					&& vusuarioR & vsede & vcredito & vdia & vhorarioR & vhora) {
				...
				if (!Arrays.equals(clave, repetir)) {
					...
					String dia = (String) vCliente.getPrp().getLista_dias().getSelectedItem();
					pareja = new Pareja(nombre, alias, "Pareja", clavefinal, correo, genero, credito, 0, hora, dia,
							sedeSelec, usuario);
					...
					if (respuesta) {
						...
						try {
							vCliente.getPrp().getLista_dias().removeItem("");
						...
						vCliente.getPrp().getThora().setText(null);
					} else {
						Estandar.MensajeError("Fallo al registrar");
					}
		...
		if (comando.equals("bPAREJASACTUALES")) {
			...
			try {
				for (Pareja pareja : listapareja.getListadeParejas()) {
					...
					if (!existePareja) {
						Object[] fila = { pareja.getNombre(), pareja.getUser(), pareja.getCorreo(), pareja.getGenero(),
								pareja.getCredito(), pareja.getDeuda() };
						modelo.addRow(fila);
					}
				}
		...
		if (comando.equals("bHISTORIAL")) {
			...
			try {
				...
				for (Compra compra : listadecompra.getListadeCompras()) {
					...
					for (int i = 0; i < rowCount; i++) {
						String horaEnTabla = vCliente.getPhc().getModel().getValueAt(i, 2).toString(); // Considerando
																										// que la hora
																										// está en la
																										// tercera
																										// columna
					...
					if (!existeHora) {
						Object[] fila = { compra.getNombre(), compra.getPrecio(), compra.getHora(), compra.getFecha(),
		...
		if (comando.equals("bSOLICITAR")) {
			...
			if (vCliente.getPs().getTmonto().getText().equals("") || monto <= 0) {
				...
				Estandar.MensajeInformacion("Ha solicitado un sobrecupo de $" + monto + " pesos",
						"Sobrecupo solicitado");
		...
		for (Producto producto : listaProductos.getListadeProductos()) {
			if (comando.equals("b" + producto.getNombre())) {
				...
				if (vSede && vCantidad) {
					Object[] opciones = { "Sí", "No" };
					int confirmar = JOptionPane.showOptionDialog(null,
							"Desea comprar " + cantidad + " " + producto.getNombre(), "Confirmar compra",
							JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE, null, opciones, opciones[1]);
					if (confirmar == JOptionPane.YES_OPTION) {
						if (totalCompra < usuario.getCredito()) {
							JOptionPane.showMessageDialog(null, "Usted ha comprado " + cantidad + " "
							...
							Compra x = new Compra(producto.getNombre(), producto.getImg(), producto.getPrecio(),
									usuario, sedecompra);
		...
		if (comando.equals("bHISTORIALpareja")) {
			...
			try {
				...
				for (Compra compra : listadecompra.getListadeCompras()) {
					...
					for (int i = 0; i < rowCount; i++) {
						String horaEnTabla = modelo.getValueAt(i, 2).toString(); // Considerando que la hora está en la
		...
		if (comando.equals("bABONARMENUpareja")) {
			if (pareja.getDeuda() == 0) {
				...
				vParejaInicio.getPap().geteDineropendiente()
						.setText("$ " + String.valueOf(pareja.getDeuda() + " pesos"));
		...
		if (comando.equals("bABONARpareja")) {
			...
			if (vParejaInicio.getPap().getTmonto().getText().equals("") || monto <= 0 || monto > pareja.getDeuda()) {
				...
				Estandar.MensajeInformacion("Ha abonado $" + vParejaInicio.getPap().getTmonto().getText() + " pesos",
						"Abono exitoso");
		...
		if (comando.equals("bCOMPRARMENUpareja")) {
			...
			// Extrae la hora como un entero
			int hora = horaActual.getHour();

			String dia = pareja.getDia();

			if (hora == pareja.getHora() && dia.equals(nombreDia)) {
				vParejaInicio.setVisible(false);
				...
				Estandar.MensajeError("No puedes realizar compras en este momento");
			}
		}
				...
				if (vSede && vCantidad) {
					...
					if (confirmar == JOptionPane.YES_OPTION) {
						if (totalCompra < pareja.getCredito()) {
							...
							Sede sedecompra = null;
							for (Sede sede : listaSedes.getListadeSedes()) {
								if (vComprapareja.getPtp().getLista_sedes().getSelectedItem()
										.equals(sede.getNombre())) {
									sedecompra = sede;
								}

							}
							Compra x = new Compra(producto.getNombre(), producto.getImg(), producto.getPrecio(), pareja,
									sedecompra);
							for (int i = 0; i < cantidad; i++) {
								listadecompra.agregarCompra(x);
							}
